/**
 * Client fetch pour l'API JSON /api/* consommée par les îlots Preact.
 * L'admin n'a pas de SSR de contenu (cahier) : ces îlots sont montés en
 * client:only et parlent aux routes API Astro, authentifiées par cookie
 * httpOnly (envoyé automatiquement par le navigateur en same-origin).
 */
