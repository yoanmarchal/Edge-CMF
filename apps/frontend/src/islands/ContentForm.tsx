...
async function loadParagraphTypes(): Promise<ParagraphTypeDef[]> {
  const { data: types } = await adminApi.get<{ data: { id: string; label: string }[] }>(
    '/api/admin/types?kind=paragraph',
  );
  const out: ParagraphTypeDef[] = [];
  for (const t of types) {
    const detail = await adminApi.get<{ success: true; data: TypeDetail }>(`/api/admin/types?id=${t.id}`);
...
export default function ContentForm({ mode, typeId, nodeId }: Props) {
    ...
    (async () => {
      try {
        const [vocabRes, pTypes] = await Promise.all([
          adminApi.get<{ data: Vocabulary[] }>('/api/admin/taxonomy'),
          loadParagraphTypes(),
        ]);
        ...
        if (mode === 'edit' && nodeId !== undefined) {
          const nodeRes = await adminApi.get<{ success: true; data: NodeDetail }>(`/api/admin/nodes?id=${nodeId}`);
          ...
          const typeRes = await adminApi.get<{ success: true; data: TypeDetail }>(
            `/api/admin/types?id=${n.contentType}`,
          );
          setTypeDetail(typeRes.data);
        } else {
          const id = typeId ?? 'page';
          const typeRes = await adminApi.get<{ success: true; data: TypeDetail }>(`/api/admin/types?id=${id}`);
          setTypeDetail(typeRes.data);
          setStatus(true);
        }
      } catch (e) {
  ...
  const submit = async (e: Event) => {
    ...
    try {
      ...
      if (mode === 'edit' && node !== null) {
        await adminApi.post('/api/admin/nodes', { _action: 'update', id: node.id, ...payload });
      } else {
        await adminApi.post('/api/admin/nodes', { _action: 'create', ...payload });
      }
      window.location.href = '/admin/content?ok=1';
    } catch (e) {
      setError((e as Error).message);
      setSaving(false);
    }
  ...
  const remove = async () => {
    ...
    try {
      await adminApi.post('/api/admin/nodes', { _action: 'delete', id: node.id });
      window.location.href = '/admin/content?ok=1';
    } catch (e) {
      setError((e as Error).message);
    }
  ...
  return (
    <>
      ...
      {mode === 'edit' && node !== null && (
        <p class="muted">
          Type : {node.contentType} — <a href={`/${node.slug}`}>voir la page</a>
        </p>
      )}
