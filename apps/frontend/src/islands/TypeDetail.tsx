...
export default function TypeDetail({ id }: { id: string }) {
  ...
  const reload = () => {
    adminApi
      .get<{ success: true; data: TypeDetailData }>(`/api/admin/types?id=${id}`)
  ...
  const addField = async (e: Event) => {
    e.preventDefault();
    try {
      await adminApi.post('/api/admin/types', {
  ...
  const deleteField = async (fieldId: string) => {
    try {
      await adminApi.post('/api/admin/types', { _action: 'delete-field', id: fieldId });
      reload();
    } catch (e) {
      setError((e as Error).message);
    }
  ...
  const deleteType = async () => {
    ...
    try {
      await adminApi.post('/api/admin/types', { _action: 'delete-type', id: detail.type.id });
      window.location.href = detail.type.kind === 'paragraph' ? '/admin/paragraphs' : '/admin/types';
    } catch (e) {
      setError((e as Error).message);
    }
  };
