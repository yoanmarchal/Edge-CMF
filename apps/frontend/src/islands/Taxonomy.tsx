...
export default function Taxonomy() {
  ...
  const reload = () => {
    adminApi
      .get<{ data: Vocabulary[] }>('/api/admin/taxonomy')
  ...
  const createVocab = async (e: Event) => {
    e.preventDefault();
    try {
      await adminApi.post('/api/admin/taxonomy', { _action: 'create-vocab', id: vid, label: vlabel });
      setVid('');
      setVlabel('');
      reload();
    } catch (e) {
  ...
  const deleteVocab = async (id: string) => {
    ...
    try {
      await adminApi.post('/api/admin/taxonomy', { _action: 'delete-vocab', id });
      reload();
    } catch (e) {
      setError((e as Error).message);
    }
  ...
  const deleteTerm = async (id: string) => {
    try {
      await adminApi.post('/api/admin/taxonomy', { _action: 'delete-term', id });
      reload();
    } catch (e) {
      setError((e as Error).message);
    }
  ...
  const createTerm = async (vocabularyId: string, label: string, slug: string) => {
    try {
      await adminApi.post('/api/admin/taxonomy', { _action: 'create-term', vocabularyId, label, slug });
      reload();
    } catch (e) {
      setError((e as Error).message);
    }
