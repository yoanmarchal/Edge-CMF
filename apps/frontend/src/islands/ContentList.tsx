...
export default function ContentList({ canWrite }: { canWrite: boolean }) {
  ...
  const reload = () => {
    Promise.all([
      adminApi.get<{ data: NodeRow[] }>('/api/admin/nodes'),
      adminApi.get<{ data: TypeRow[] }>('/api/admin/types?kind=node'),
    ])
      .then(([n, t]) => {
        setNodes(n.data);
        setTypes(t.data);
  ...
  const remove = async (id: string) => {
    ...
    try {
      await adminApi.post('/api/admin/nodes', { _action: 'delete', id });
      reload();
    } catch (e) {
    ...
    <>
      ...
      {canWrite && types.length > 0 && (
        <p class="action-row">
          <span>Créer :</span>
          {types.map((t) => (
            <a class="badge" href={`/admin/content/new?type=${t.id}`}>
              + {t.label}
            </a>
          ))}
        </p>
          ...
          <table>
            ...
            <tbody>
              {nodes.map((n) => (
                <tr>
                  <td>
                    <a href={`/admin/content/edit/${n.id}`}>{n.title}</a>
                  </td>
                  <td>{n.contentType}</td>
