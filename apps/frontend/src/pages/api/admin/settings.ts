...
/** API JSON /api/admin/settings — réglages clé/valeur (thème actif, etc.). */
export const GET: APIRoute = async ({ locals, cookies, url }) => {
  ...
  return contentClient(env).api.settings[':key'].$get({ param: { key: key.data } });
};
...
export const PUT: APIRoute = async ({ request, locals, cookies }) => {
  ...
  return contentClient(env).api.settings[':key'].$put({
    param: { key: key.data },
