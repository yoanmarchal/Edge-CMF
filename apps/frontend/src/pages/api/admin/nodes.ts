...
/**
 * API JSON /api/admin/nodes — consommée par l'îlot Preact ContentList/ContentForm
 * (client:only, l'admin n'a plus besoin de SSR). Fait office de proxy
 * authentifié vers le content-worker (Service Binding, jamais exposé
 * directement au client). Mêmes règles d'accès que l'ancien formulaire
 * server-rendered, réponses en JSON au lieu d'une redirection 303.
 */
export const GET: APIRoute = async ({ locals, cookies, url }) => {
  ...
  if (id !== null) {
    ...
    return client.api.node[':id'].$get({ param: { id: parsedId.data } });
  }

  const query: Record<string, string> = { all: '1', limit: '100' };
  for (const [key, value] of url.searchParams) query[key] = value;
  return client.api.nodes.$get({ query });
};
